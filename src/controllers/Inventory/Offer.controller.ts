...
class OfferController {
  ...
  async createOffer(req: Request, res: Response): Promise<void> {
    try {
      ...
      const createdOffer: OfferDTO =
        await this.offerService.createOffer(offerInput);
  ...
  async updateOffer(req: Request, res: Response): Promise<void> {
    try {
      ...
      const offerInput: OfferInputDTO = req.body;
      const updatedOffer: OfferDTO | null = await this.offerService.updateOffer(
        offerId,
        offerInput,
      );
      if (updatedOffer) {
  ...
  async getOfferById(req: Request, res: Response): Promise<void> {
    try {
      const offerId: string = req.params.id;
      const offer: OfferDTO | null =
        await this.offerService.getOfferById(offerId);
      if (offer) {
      ...
      res
        .status(500)
        .json({ message: "Failed to get offer", error: error.message });
    }
  }
  ...
  async getAllOffers(req: Request, res: Response): Promise<void> {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to get offers", error: error.message });
    }
  }
}
