...
async deleteCartProduct(req:Request, res: Response): Promise<void> {
  try {
    ...
    const deletedCartItem = await this.cartService.deleteCartProduct(productId,CartID)
   
      res.status(200).json({message: "Product has been deleted"});
 
  } catch (error: any) {
    res.status(500).json({ message: "Failed to delete to cart", error: error.message });
  }
}
async getCartByUserId(req: Request, res: Response): Promise<void> {
    try {
      ...
      const cartItem: cartDTO | null = await this.cartService.getCartByUserId(userId);
      if (cartItem) {
  ...
  async updateCart(req: Request, res: Response): Promise<void> {
    try {
      const CartID: string = req.params.id;
      const cartInput: cartInputDTO = req.body;
      const updateCart: cartDTO | null = await this.cartService.updateCart(CartID, cartInput);
      if (updateCart) {
