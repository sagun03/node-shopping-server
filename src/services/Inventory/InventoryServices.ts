...
class InventoryService {
  public async createInventory(inventoryInput: InventoryInputDTO): Promise<InventoryDTO> {
    const newInventory = new Inventory(inventoryInput);
    const savedInventory = await newInventory.save();
    return this.mapInventoryToDTO(savedInventory);
  }

  public async updateInventory(inventoryId: string, inventoryInput: InventoryInputDTO): Promise<InventoryDTO | null> {
    const updatedInventory = await Inventory.findByIdAndUpdate(inventoryId, inventoryInput, { new: true });
  ...
  public async getInventoryById(productId: string): Promise<InventoryDTO | null> {
