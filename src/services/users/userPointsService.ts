...
class UserPointsService {
    ...
    async createEntry(transact: any) : Promise<userPointsDTO> {
        ...
        if(!document) 
            ...
            const pointsDetail = new userPointsDTO(transact.userId, 
                transact.transactionType, document?.pointsBalance,
            document?.role, document?.referralUserId, transact.reason)
